...
RegisterDialog::RegisterDialog(QWidget *parent) : QDialog(parent), ui(new Ui::RegisterDialog),_countdown(5) {
  ...
  ui->pass_visible->setCursor(Qt::PointingHandCursor); // 预定义的光标形状，通常显示为一只指向的手。
...
void RegisterDialog::initHttpHandlers() {
  ...
  _handlers.insert(ReqId::ID_REG_USER, [this](QJsonObject jsonObj){
    ...
    if(error != ErrorCode::SUCCESS){
      if(error == ErrorCode::UserExist ) showTip(tr("该用户已经被注册过了"), false);
