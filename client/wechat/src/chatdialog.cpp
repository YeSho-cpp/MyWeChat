    ...
    _state(ChatUIMode::ChatMode),_last_widget(nullptr),_cur_chat_uid(0)
{
  ...
  clearAction->setIcon(QIcon(":/close_transparent.png"));
  // 初始时不显示清除图标
  ...
  connect(ui->search_edit, &QLineEdit::textChanged, [clearAction](const QString &text) {
    if (!text.isEmpty()) {
      clearAction->setIcon(QIcon(":/close_search.png"));
    } else {
      clearAction->setIcon(QIcon(":/close_transparent.png")); // 文本为空时，切换回透明图标
    }
  ...
  connect(clearAction, &QAction::triggered, [this, clearAction]() {
    ...
    clearAction->setIcon(QIcon(":/close_transparent.png")); // 清除文本后，切换回透明图标
...
void ChatDialog::slot_text_chat_msg(const std::shared_ptr<TextChatMsg>& msg)
{
...
void ChatDialog::slot_append_send_chat_msg(const std::shared_ptr<TextChatData>& msgdata) {
  if (_cur_chat_uid == 0) {
    return;
  }
...
void ChatDialog::slot_friend_info_page(std::shared_ptr<UserInfo> user_info)
{
  ...
  ui->friend_info_page->SetInfo(std::move(user_info));
}
